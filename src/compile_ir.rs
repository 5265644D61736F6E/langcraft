...
fn compile_ashr(
    ...
    if let Some(value) = as_const_64(operand1) {
        ...
        if op0.len() == 1 {
            ...
            cmds.push(
                McFuncCall {
                    id: McFuncId::new("intrinsic:ashr128"),
                }
                .into(),
            );
            cmds.push(assign(dest.next().unwrap(), param(0, 0)));
                        ...
                        if target.len() == 2 && source.len() == 2 {
                            ...
                            match predicate {
                                ...
                                IntPredicate::SGT | IntPredicate::SLT => {
                                    let relation = if matches!(predicate,IntPredicate::SGT) {
                                        cir::Relation::GreaterThan
                                    } else if matches!(predicate,IntPredicate::SLT) {
                                        cir::Relation::LessThan
                                    } else {
                                        dumploc(debugloc);
                                        unreachable!("Handled {:?} as SGT/SLT",predicate)
                                    };
                                    
                                    let ugt = get_unique_holder();
                                    cmds.push(assign_lit(dest.clone(),0));
                                    
                                    let mut base_cmd = Execute::new();
                                    base_cmd.with_if(ExecuteCondition::Score {
                                        target: target[3].clone().into(),
                                        target_obj: OBJECTIVE.to_string(),
                                        kind: ExecuteCondKind::Matches((..=-1).into())
                                    });
                                    base_cmd.with_run(assign_lit(dest.clone(),1));
                                    cmds.push(base_cmd.into());
                                    
                                    let mut eq_3 = Execute::new();
                                    let mut eq_2 = Execute::new();
                                    let mut eq_1 = Execute::new();
                                    
                                    eq_3.with_if(ExecuteCondition::Score {
                                        target: source[3].clone().into(),
                                        target_obj: OBJECTIVE.into(),
                                        kind: ExecuteCondKind::Relation {
                                            relation: cir::Relation::Eq,
                                            source: source[3].clone().into(),
                                    ...
                                    eq_2.with_if(ExecuteCondition::Score {
                                        target: source[2].clone().into(),
                                        target_obj: OBJECTIVE.into(),
                                        kind: ExecuteCondKind::Relation {
                                            relation: cir::Relation::Eq,
                                            source: source[2].clone().into(),
                                    ...
                                    eq_1.with_if(ExecuteCondition::Score {
                                        target: source[1].clone().into(),
                                        target_obj: OBJECTIVE.into(),
                                        kind: ExecuteCondKind::Relation {
                                            relation: cir::Relation::Eq,
                                            source: source[1].clone().into(),
                                    ...
                                    cmds.extend(compile_unsigned_cmp(target[3].clone(),source[3].clone(),ugt.clone(),
                                        relation.clone()));
                                    cmds.extend(compile_unsigned_cmp(target[2].clone(),source[2].clone(),ugt.clone(),
                                        relation.clone()).into_iter().map(|cmd| {
                                            let mut exec = eq_3.clone();
                                            exec.with_run(cmd);
                                            
                                            Command::Execute(exec)
                                        }));
                                    cmds.extend(compile_unsigned_cmp(target[1].clone(),source[1].clone(),ugt.clone(),
                                        relation.clone()).into_iter().map(|cmd| {
                                            let mut exec = eq_2.clone();
                                            exec.with_run(cmd);
                                            
                                            Command::Execute(exec)
                                        }));
                                    cmds.extend(compile_unsigned_cmp(target[0].clone(),source[0].clone(),ugt.clone(),
                                        relation).into_iter().map(|cmd| {
                                            let mut exec = eq_1.clone();
                                            exec.with_run(cmd);
                                    ...
                                    cmds.extend(vec![
                                        make_op(dest.clone(),"+=",ugt.clone()),
                                        make_op(dest.clone(),"%=",ScoreHolder::new("%%2".to_string()).unwrap())
                                    ]);
                                },
                                IntPredicate::ULT => {
                                    let relation = cir::Relation::LessThan;
                                    ...
                                    cmds.extend(compile_unsigned_cmp(target[3].clone(),source[3].clone(),dest.clone(),
                                        relation.clone()));
                                    cmds.extend(compile_unsigned_cmp(target[2].clone(),source[2].clone(),dest.clone(),
                                        relation.clone()).into_iter().map(|cmd| {
                                            let mut exec = eq_3.clone();
                                            exec.with_run(cmd);
                                    ...
                                    cmds.extend(compile_unsigned_cmp(target[1].clone(),source[1].clone(),dest.clone(),
                                        relation.clone()).into_iter().map(|cmd| {
                                            let mut exec = eq_2.clone();
                                            exec.with_run(cmd);
                                    ...
                                    cmds.extend(compile_unsigned_cmp(target[0].clone(),source[0].clone(),dest.clone(),
                                        relation).into_iter().map(|cmd| {
                                            let mut exec = eq_1.clone();
                                            exec.with_run(cmd);
